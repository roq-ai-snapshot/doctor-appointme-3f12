...
import { getNewTables } from 'apiSdk/new-tables';
...
import { MedicalStaffListPage as ShowCaseList } from 'pages/medical-staffs';

function DashboardPage() {
  const { session } = useSession();

  const newTableStat = useSWR<PaginatedInterface<NewTableInterface>>(
    () => `/new-tables`,
    () => getNewTables({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'New Tables',
      result: newTableStat?.data?.totalCount,
      isLoading: newTableStat.isLoading,
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Medical Staffs
                </Text>
                <NextLink href={`medical-staffs`} passHref legacyBehavior>
                  <Button
